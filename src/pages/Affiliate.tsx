...
export const Affiliate = () => {
  ...
  const handleCopy = async () => {
    try {
      ...
      notify("Ne mogu kopirati link (clipboard blokiran).", "error");
    }
  };
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <header>
        <h2 className="text-4xl font-black italic uppercase tracking-tighter text-white">
          Partner <span className="text-[#39de7f]">Program</span>
        </h2>
        <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.4em] mt-2 italic">
          Monetize your network with Fate
        </p>
      </header>

      {/* STATISTICS GRID */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatItem icon={MousePointer2} label="Total Clicks" value="1,842" colorClass="text-blue-500" />
        <StatItem icon={UserPlus} label="Registrations" value="124" colorClass="text-purple-500" />
        <StatItem icon={TrendingUp} label="Conv. Rate" value="6.7%" colorClass="text-yellow-500" />
        <StatItem icon={Wallet} label="Total Earned" value="$1,420.00" colorClass="text-[#39de7f]" />
      </div>
      ...
      <div className="bg-[#080808] border border-white/5 p-10 rounded-[3rem] relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <h3 className="text-xl font-black italic uppercase text-white mb-6 tracking-tight">
          ...
          <p className="text-gray-600 text-[10px] font-bold uppercase mt-6 tracking-widest italic">
            You receive <span className="text-[#39de7f]">25% commission</span> for every successful
      ...
      <div className="bg-[#080808] border border-white/5 p-8 rounded-[2.5rem] mt-8">
        <div className="flex justify-between items-end mb-4">
          <div>
            <p className="text-[10px] font-black text-gray-500 uppercase italic tracking-widest">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-white/[0.02]">
              <tr>
                <th className="px-8 py-4 text-[9px] font-black uppercase text-gray-500 tracking-widest italic">
                  User
                </th>
                <th className="px-8 py-4 text-[9px] font-black uppercase text-gray-500 tracking-widest italic">
                  Date
                </th>
                <th className="px-8 py-4 text-[9px] font-black uppercase text-gray-500 tracking-widest italic text-right">
                  Commission
                </th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-white/5">
              {recent.map((row, i) => (
                <tr key={`${row.user}-${i}`} className="hover:bg-white/[0.01] transition-colors">
                  <td className="px-8 py-5 text-white font-bold text-xs italic uppercase tracking-tighter">
                    {row.user}
                  </td>
                  <td className="px-8 py-5 text-gray-500 text-[10px] font-black uppercase">
                    {row.date}
                  </td>
                  <td className="px-8 py-5 text-[#39de7f] font-black text-xs text-right italic">
                    {row.amount}
                  </td>
                </tr>
              ))}
...
const StatItem = ({ icon: Icon, label, value, colorClass }: StatItemProps) => (
  <div className="bg-[#080808] border border-white/5 p-8 rounded-[2.5rem] hover:border-white/10 transition-all">
    <div className="flex justify-between items-start mb-4">
      <span className="text-[9px] font-black uppercase text-gray-600 tracking-widest italic">
        {label}
      </span>
      <div className={`p-2 bg-black rounded-lg border border-white/5 ${colorClass}`}>
        <Icon size={16} />
      </div>
    </div>
